...
describe("User Registration UT/IT v1", () => {
    ...
    it("UT2-008 - Password and Verify password did not match (password != verify password)", async () => {
        const user = await signup("junrel","junrel@awsys-i.com","Alje9999","alje@999");
        expect(user).toBe(400);
    })
    it("UT2-009 - User's registered successfully", async () => {
        const user = await signup("junrel.ednalig","junrel@awsys-i.com","June@123","Jun@123");
        expect(user).toBe(200);
    })
    it("UT2-010 - User's record already exist", async () => {
